...
- **Management Results Deck**: An executive `.pptx` built from the consolidated results — headline production/risk, a methodology-in-brief slide, per-segment charts, the swap-in/swap-out bridge, acceptance change, a 2D + 3D risk-surface slide **per segment and supersegment**, and out-of-time validation — with a data-driven recommendation narrative.
...
| `generate_results_presentation.py` | Management results deck (.pptx) from the consolidated outputs |
| `generate_presentation.py` | Methodology presentation (.pptx / .pdf) — how the pipeline works |
...
| `presentation_charts.py` | Management summary charts (matplotlib) + 2D heatmap + 3D PNG export for the results deck |
...
| `results_deck/Credit_Risk_Results_{scenario}.pptx` | Management results deck (+ `images/`). Written by `generate_results_presentation.py` |
...
| **Cutoff Grids** | Per-segment score-cutoff acceptance grids (one coloured grid per scenario, faceted by income-bin slice for N>2): green **A** accept / red **R** reject / grey **—** unobserved cell, with a **thick navy frontier border** tracing the accept/reject boundary. The MR-period grid is drawn only when it differs from main — a frozen MR mask (`mr_reoptimize_cutoffs=false` or a fixed-cutoff segment) reproduces the main accept/reject and is skipped |
