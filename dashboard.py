...
def get_scenarios():
    ...
    for f in files:
        # Match pattern: risk_production_summary_table_1.0.csv
        # Also handle risk_production_summary_table.csv (Base)
        ...
        elif filename == "risk_production_summary_table.csv":
...
def get_comparison_data():
    ...
    for s in scenarios:
        ...
        try:
            ...
            # Target Risk (parse from scenario name or base config)
            try:
                if s == "Base":
                    row['Target_Risk'] = 1.1 # Default/Fallback
                else:
                    row['Target_Risk'] = float(s)
            except:
                row['Target_Risk'] = None
...
def render_content(active_tab, scenario):
    if active_tab == "tab-comp":
        ...
        fig_sensitivity = px.line(
            df_comp.sort_values("Target_Risk"), x="Target_Risk", y=["Main_Risk", "MR_Risk"],
            title="Risk Sensitivity: Target vs Realized (Main & MR)",
    ...
    suffix = "" if scenario == "Base" else f"_{scenario}"
    ...
    if active_tab == "tab-main":
        return html.Div([
            html.H4(f"Main Period Summary (Risk: {scenario})", className="my-3"),
            load_table(main_csv),
            html.Hr(),
        ...
        return html.Div([
            html.H4(f"MR Period Summary (Risk: {scenario})", className="my-3"),
            load_table(mr_csv),
            html.Hr(),
