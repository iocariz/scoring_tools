...
def apply_plotly_style(fig: go.Figure, title: str = None, height: int = None, width: int = None):
    ...
    layout_update = dict(
        font=dict(family="Arial, sans-serif", size=12, color=COLOR_PRIMARY),
        title_font=dict(family="Arial, sans-serif", size=20, color=COLOR_PRIMARY, weight="bold"),
        plot_bgcolor="white",
        paper_bgcolor="white",
        # Axis defaults
        xaxis=dict(
        ...
        yaxis=dict(showgrid=True, gridcolor="#EEF0F4", gridwidth=1, linecolor="#D1D5DB", linewidth=1, zeroline=False),
        # Legend defaults
        legend=dict(
