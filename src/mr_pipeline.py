...
def calculate_metrics_from_cuts(
    ...
    optimal_solution_df: Optional[pd.DataFrame],
    variables: List[str]
...
def process_mr_period(
    ...
    try:
        ...
        data_mr_period = filter_by_date(
            data_clean,
            'mis_date',
            config_data['date_ini_book_obs_mr'],
            config_data['date_fin_book_obs_mr']
        )
        ...
        fig_mr.add_trace(go.Surface(
            x=data_surf_pivot_mr.columns, y=data_surf_pivot_mr.index, z=data_surf_pivot_mr.values, colorscale='turbo'
        ))
