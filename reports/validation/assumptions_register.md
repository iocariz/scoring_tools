...
| `optimum_risk` | per-segment (`segments.toml`; e.g. no_premium_cd = 1.40) | 1.1 | `config.py` / `segments.toml` | **Core** | The risk target (%) the optimizer selects against (b2 units). Defines the policy. | Model owner / portfolio policy |
| `risk_indicator` | `b2_ever_h6` | `b2_ever_h6` | `config.py` | **Core** | Which indicator the optimizer targets: `b2_ever_h6` (classic) or `hri_h6` (Harmonized Risk Indicator, `h_num_h6/h_den_h6`, no multiplier). Both are always reported; switching the target changes cutoffs and requires re-validation. In hri mode: model reuse refused, MR keeps the frozen main mask, b2-basis bootstrap CIs skipped (HRI CIs pending). | Model owner / portfolio policy |
| `optimum_hri` | unset | unset (required iff `risk_indicator="hri_h6"`) | `config.py` | **Core** | The HRI target (%) when HRI is the optimization target. HRI-% units — NOT comparable to `optimum_risk`'s ×7-annualised b2 scale. `risk_step`/resimulation targets follow the selected indicator's units. | Model owner / portfolio policy |
...
| `mr_reoptimize_cutoffs` | `true` | `true` | `config.py` | **Expert** | MR-period cutoff handling. `true` (legacy) **re-optimizes** the mask on the recalibrated MR surface (targeting the main b2), so MR risk lands ≈ the target by construction and the cutoffs can drift from main. `false` **freezes** the main mask in MR (honest "chosen policy on the newer cohort" view; e.g. no_premium-style fixed-cutoff segments showed +0.4pp MR risk once frozen, closer to the M4 OOT realized). **Fixed-cutoff segments are always frozen regardless** (a fixed policy must stay fixed; bug-fix — they used to silently re-optimize). Flipping it changes MR headline numbers ⇒ governed change. Validation/monitoring-only (does not feed the chosen cutoffs). | Governed |
