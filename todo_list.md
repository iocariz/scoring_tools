...
63. **Partial fix (R2 part 4):** Introduced `OptimizationResult` dataclass (same `@dataclass` pattern as `PreprocessingResult`), replacing the 7-tuple return from `run_optimization_phase`. Supports attribute access (`opt.data_summary`) and keeps positional unpacking for legacy call sites via `__iter__`. Renamed `_build_scenario_list` → `build_scenario_list`, `_compute_mr_annual_coef` → `compute_mr_annual_coef`, `_save_cutoff_summaries` → `save_cutoff_summaries` (they were imported as privates from `main.py` — now public-named). Both call sites in `main.py` migrated to attribute access. **Remaining:** the actual file split into `cutoff_optimization.py` / `scenarios.py` / `sensitivity.py` is a multi-session refactor that requires test coverage on each phase first — tracked as follow-up scope. The dataclass + public helpers are the enabling foundation.
