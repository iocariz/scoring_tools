...
51. ~~**No lockfile committed** (`pyproject.toml`) — Only lower-version bounds; `uv.lock` not in git. `pip install` silently picks up CVE-laden transitive bumps. **Fix:** Commit `uv.lock`, run `uv lock --check` in CI.~~ **Partially obsolete / Fixed:** `uv.lock` was in fact tracked already (committed in `01440b9`); the audit statement was stale. Added `uv lock --check` step to the CI lint job so any future drift between `pyproject.toml` and `uv.lock` breaks the build.
