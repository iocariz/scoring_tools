...
63. ~~**`src/pipeline/optimization.py` is not "thin orchestration"** (1,250 lines, 4 substantive phases).~~ **Fully fixed across R2a + R2b-iv.** R2a (PR #29) introduced the `OptimizationResult` dataclass (same pattern as `PreprocessingResult`), renamed the 3 private helpers to public names, and migrated all call sites. R2b-iv (PR #33) completed the work: split the 1315-line module into four focused modules — `cutoff_optimization.py` (511 ln: `OptimizationResult` + `run_optimization_phase`), `scenarios.py` (493 ln: 4 scenario-analysis helpers), `sensitivity.py` (108 ln), `ri_optimizer.py` (247 ln). `optimization.py` is now a 39-line re-export shim that keeps the legacy import path working for backward compat. Tests that monkey-patched helpers on the old module now target `cutoff_optimization` — the shim deliberately does not re-expose internal imports.

64. ~~**Three competing dashboards duplicate ~15–20 KB of code** (`dashboard.py` 175 KB Dash, `gradio_dashboard.py` 48 KB Gradio, `interactive_allocator.py` 22 KB Dash) — Each redefines `get_data_dir`, `get_images_dir`, `get_available_segments`, `get_scenarios`, `parse_coefficients`, `_resolve_supersegment`, `load_cutoff_data`. **Fix:** Extract `src/dashboard_data.py`; have all three depend on it. Reassess whether `gradio_dashboard.py` is still needed.~~ **Fixed (R2 part 5):** Extracted `src/dashboard_data.py` with `get_data_dir`, `get_images_dir`, `get_models_dir`, `get_available_segments`, `get_scenarios`, `get_scenario_paths`, `resolve_supersegment`, `parse_coefficients`, and `SCENARIO_ORDER`. Functions are pure (take `output_base: Path` as a parameter rather than reading a module-level constant). `dashboard.py` and `gradio_dashboard.py` now import from the shared module via thin module-level wrappers that forward to it, preserving each dashboard's `OUTPUT_BASE` convention and keeping internal call sites unchanged. Net -139 lines across the two dashboards. 29 unit tests pin the behaviour. `interactive_allocator.py` doesn't duplicate these helpers (uses its own narrower `get_available_scenarios`) so no migration there. `gradio_dashboard.py` kept — whether to retire it is a product decision, not architectural, tracked separately in reporting feature #64 (PR #28 comment).

65. ~~**Partial fix (R2 part 3):** Introduced `CutoffSpec` frozen dataclass in `src/optimization_utils.py` with `.from_cut_map(...)` / `.from_mask(...)` constructors, unified `.classify(df)` dispatch, and `.as_2d_cut_map()` adapter for call sites that genuinely need the 2-var shape. Construction refuses to mix encodings. Migrated `_bootstrap_worker` / `calculate_bootstrap_intervals` as the proof-of-concept path (legacy kwargs still accepted — auto-convert to `CutoffSpec` internally; new callers can pass `spec=` directly). Hardcoded fallback variable list extracted from `src/consolidation.py` line 3536 into the module-level `_FALLBACK_REPORTING_VARIABLES` constant with documentation explaining it's a last-resort for malformed configs, not a starting point for new deployments.~~ **Fixed (R2b-vii):** Added `CutoffSpec.from_optimal_solution(...)` classmethod and migrated the four core cut_map-construction call sites: `src/mr_pipeline.py::calculate_metrics_from_cuts`, `src/audit.py::generate_audit_table`, `src/pipeline/scenarios.py::run_scenario_analysis`, and `src/consolidation.py`'s audit-summary block. Net ~125 duplicated lines removed. Remaining per-module migrations in `src/inference_optimized.py`, `src/sensitivity.py`, `src/reject_inference.py`, `src/plots.py` are low-value (data flow doesn't need the dispatch — they consume already-classified results) and are not tracked further.
