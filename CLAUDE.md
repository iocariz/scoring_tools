...
**Audit P2 corrections (2026-09-08):** New trainings save the exposure companion and its SHA-256 sidecar inside the versioned risk-model directory returned by persistence; older new-format pairs remain reusable after subsequent trainings. Policy registration and comparison share the full grid-and-accepted-set ID builder. Sensitivity perturbations, targets, flip thresholds and marginal risks use `settings.selected_indicator` / `selected_target`, including HRI's multiplier 1; swap-in risk caps retain their b2 basis. RI monotonicity now solves demand-weighted least squares on the original clipped multipliers over the complete cell partial order, using SciPy SLSQP with canonical coordinate ordering and convergence/feasibility checks. The previous axis-wise warm-start plus greedy block pooling was feasible but not the least-squares solution on branching grids (F11).

**Model training:** `cv_folds`. `z_threshold` (float, default `3.0`, `ge=0`, audit #56) — modified-z threshold for target-based outlier handling in the risk fit. Extreme-risk bins are **winsorized** (their `b2_ever_h6` target clipped to the `±z_threshold` boundary), NOT dropped — the old drop discarded the high-risk tail from the fitted surface, letting the smooth model extrapolate lower risk into exactly the cells where the cutoff is most sensitive (anti-conservative). Set `z_threshold=0` to disable entirely; smaller = more aggressive clipping. Now threaded from config into `inference_pipeline` (was hard-coded `DEFAULT_Z_THRESHOLD`). Changing it moves the fitted surface and therefore cutoffs — validate on real data. `model_hurdle_per_loan` (bool, default `false`, Expert/default-off, audit #6) — when `true`, a two-part `HurdleRegressor` is offered as a model candidate, trained on **per-loan** data (real zero mass in the default indicator) with exposure-weighted severity, and scored on the same bin-level CV RMSE as the other candidates; it is skipped automatically when the per-loan zero mass is degenerate (∉ [2%, 99.9%]). When `false` the hurdle is not offered at all — on the bin-aggregated target it degenerates to plain Ridge/Lasso. Enabling it can change the selected risk model and therefore the cutoffs (validate on real data first).
...
**H3 metrics:** `multiplier_h3` (scaling factor for 3-month risk), `use_mr_outcomes` (enable H3→H6 extrapolation), `mr_min_obs_per_bin`, `mr_extrapolation_method` (`"linear"` / `"power"` / `"logistic"` / `"auto"`), `mr_extrapolation_curvature` (power exponent; ignored when method is `"auto"` — curvature is fitted from main-period data via weighted log-log regression in `fit_h3_extrapolation_curve`), `mr_maturity_months` (min months since booking for H6 maturity filter), `mr_extrapolation_risk_multiplier` (relative ceiling for MR risk), `mr_extrapolation_hard_cap` (absolute ceiling for risk %). `mr_reoptimize_cutoffs` (bool, default `true`, Expert) — when `true` the inline MR check **re-optimizes** the acceptance mask via MILP on the recalibrated MR risk surface (targeting the main b2), so MR cutoffs can drift from main and MR risk lands ≈ the target by construction; when `false` the MR period **keeps the main (frozen) mask** and only recomputes risk/production on those cells (honest "how does my chosen policy do on the newer cohort", same basis as M4 but with H3→H6 extrapolation). **Fixed-cutoff segments are ALWAYS frozen in MR regardless of this flag** (a deliberately-fixed policy must stay fixed — the flag only governs MILP-optimized segments). Flipping it changes MR headline numbers (governance-relevant). When the MR mask actually re-optimizes, `_write_mr_cutoff_drift_overlay` emits a 2-var acceptance grid (`mr_cutoff_drift*.html`, embedded in the segment report's MR section) colouring each cell **kept** / **tightened** (main-only) / **loosened** (MR-only) / **rejected** so the cutoff drift is visible. Independently, the MR period emits its own cell-level acceptance grid (`cutoff_summary_wide_mr*.csv`, via `generate_cutoff_summary` on the MR mask+grid), rendered **next to the main-period grid** in both the segment report's MR Validation section and the consolidated Excel Exec Summary (`<seg> — MR period`) — so a `mr_reoptimize_cutoffs=true` run shows the main grid *and* the re-optimized MR grid. All acceptance grids are **three-state**: cells render **A** (accept) / **R** (reject) / **— N/A** (unobserved phantom cell — no records in the cohort), driven by an `observed` flag in `generate_cutoff_summary` that sets `accepted=NA` for cells absent from the data; this keeps empty cells on the smaller MR cohort / finer grids from being mislabeled as rejections (the renderers already drew NA neutrally). The N/A state is **conditional** — a grid showing only A/R is correct (every cell in that cohort is observed), not a missing feature; e.g. on the consolidated Exec Summary the `known` MR grid exercises all three states while `new`'s MR grid is A/R-only. Verified (2026-06): the Exec Summary shows a main grid **and** a `<seg> — MR period` grid per segment, each MR grid cross-checks cell-for-cell against `cutoff_summary_wide_mr.csv`, and the main vs MR grids are distinct (e.g. `known` 215 vs 209 accepted cells) — confirming the MR grid is the re-optimized mask, not a copy of main. When the MR mask re-optimizes, the MR audit is **regenerated on the re-optimized mask** inside `process_mr_period` so the production reconcile (`reconcile_risk_production_summary_with_audit` overwrites only `Production (€/%)` from the audit) and the **persisted** `audit_<scenario>_mr.csv` both align with the re-optimized mask — i.e. MR risk, production, the saved audit, and the grid all sit on the *same* cutoff. (Previously the audit was built on the main mask in `scenarios.py` *before* re-optimization, so production silently reflected the main cutoff while risk reflected the MR one.) `save_audit_tables` now runs **before** `process_mr_period` so the MR re-save (re-optimized mask) is the final write to `audit_*_mr.csv`; freeze/fixed-cutoff/baseline paths keep the main-mask audit, which is correct there.

**Out-of-time backtest (M4):** `run_backtest.py` + `src/backtest.py` apply a completed run's **frozen** accepted-cell set (`accepted_cells*.csv`) **as-is** to a held-out out-of-time cohort and compare realized vs predicted risk/production. Distinct from the inline MR check (`mr_pipeline.process_mr_period`), which by default **re-optimizes** the mask and is risk-only (toggle via `mr_reoptimize_cutoffs`; always frozen for fixed-cutoff segments) — the backtest never re-fits bins/model/mask (read-only; writes only under `--output`). The held-out window is auto-derived as `(date_fin_book_obs, max(mis_date) − maturity_months]` (the post-training cohorts old enough to have a realized H6; trades recency for maturity, no H3 extrapolation). Risk is compared directly (a rate); the clean same-basis drift signal is **in-sample realized → OOT realized** (both booked-only under the identical cutoff) — `predicted` (from `optimal_solution*.csv`) carries reject-inference + stress conservatism, so it is a different basis. Production is reported as rates (acceptance %, realized €), since absolute € is not comparable across periods of different size. Per-cell calibration (predicted vs realized per accepted cell, with booked counts) flags thin cells. The OOT risk carries a **bootstrap CI** and the per-segment flag is **noise-aware**: `DRIFT` only when there are ≥`MIN_DEFAULTS_FOR_DRIFT` (10) OOT defaults AND the OOT CI sits entirely above the in-sample CI; `INCONCLUSIVE` when too few defaults; `OK` when CIs overlap (added after a 2026-06 investigation found the no_premium_cd/ef drift was within sampling noise on a ~2-cohort window — not a maturity artifact). `run_batch` runs the backtest **automatically** after segments (gated by `--no-backtest`, reusing the loaded data) so the consolidated report's out-of-time sheet is always populated; `backtest_all` (in `run_backtest.py`) is the shared orchestration.

**Consolidated report — trust layer.** `consolidated_risk_production.xlsx` (`export_consolidated_excel`, `src/consolidation.py`) surfaces the validation evidence for credit-risk/policy consumers: Exec-Summary KPI cards show **bootstrap CI bands** + a plain-language "Recommendation & key risks" narrative; a **"Validation & Governance"** sheet (data snapshot SHA/git/config from lineage, key assumptions + governance tier incl. `multiplier`-FIXED, per-segment reproducibility + PSI/stability status, sign-off pointer); and an **"Out-of-time Validation"** sheet (M4 backtest: predicted vs realized + CIs + the noise-aware flag). All trust-layer readers degrade gracefully (a missing artifact shows a note, never crashes the workbook). `scripts/check_cutoff_frontier.py` verifies every grid on the Cutoff Grids sheet has a continuous, staircase-shaped accept/reject frontier (closed thick-border chains, no gaps across grey cells, monotone accept region — the #219/#220 invariants; exit 1 on failure); the same checks run in CI on synthetic grids via `tests/test_check_cutoff_frontier.py`.

**Harmonized Risk Indicator (HRI).** A second risk metric shown BESIDE b2 everywhere: `hri_h6 = h_num_h6/h_den_h6`, `hri_h3 = h_num_h3/h_den_h3` — plain ratios, **no multiplier** (registry + `RiskIndicator` spec in `src/risk_indicators.py`; `hri_available`/`hri_h3_available` are the canonical fail-soft gates). The `h_*` source columns are OPTIONAL (in `config.toml` `indicators`; present in the direct extract, absent in older ecom/retail ones): when missing they are stripped from settings with one warning and HRI simply doesn't appear anywhere (mirrors the optional-H3 pattern; shared `OPTIONAL_INDICATOR_COLUMNS` frozensets in `constants.py`). Surfaces: "HRI (%)" / "HRI H3 (%)" columns on the main + MR risk-production summary tables (raw `h_*` sums persisted like the todu pair so consolidation re-aggregates Σnum/Σden — never rate averages), consolidated CSV (`*_hri_pct`, conditional like the H3 block), Excel (formatted %, conditional HRI KPI card row — b2-only workbooks keep the exact old layout), consolidated HTML portfolio table, segment HTML (raw `h_*` hidden via `_exclude_todu`). Swap-in/repesca HRI is **NaN by design** (rejected loans carry no HRI outcomes; no model fills them yet) and renders "—", never 0.00% — suffixed HRI columns deliberately skip b2's `.fillna(0)`. RI uplift and stress do NOT touch `h_num_*` while HRI is display-only. The MR H3-floor clamp back-solves only `todu_30ever_h6` — regression-tested to never touch HRI. GOTCHA: `consolidate_segments` ends with a hard-coded `column_order` whitelist that silently drops unknown `to_dict` keys (this is why `*_risk_h3_pct` never reached the consolidated CSV) — new metric families must be added there. **Selectable optimization target (phase 2, Expert/governance-relevant):** `risk_indicator` (`"b2_ever_h6"` default | `"hri_h6"`) selects the indicator the OPTIMIZER targets; `optimum_hri` (required with hri, HRI-% units — NOT comparable to `optimum_risk`'s b2 scale) is the target, and `risk_step`/`--resimulate` are interpreted in the selected indicator's units. In hri mode: MILP coefficients/frontier ordering/scenario selection run on `hri_h6` (both indicators still computed on every frontier row; the swap-in risk cap `max_swapin_risk` stays b2-unit); an HRI model pair additionally trains (hri rate model + `h_den_h6` exposure model, stashed on the `risk_inference` dict) so repesca cells get modeled HRI, and RI uplift + stress then DO scale `h_num_h6` (display-only runs never do); the b2 model still trains for display continuity. Hard guards, never silent degradation: missing `h_*` columns with hri selected → DataValidationError at load; `--model-path`/`--reuse-models` refused (saved dirs carry only the b2 pair); GA/legacy fallbacks refuse hri (`RuntimeError`); **MR keeps the frozen main mask in hri mode** (`mr_reoptimize_cutoffs` ignored with a warning — the MR recalibration/extrapolation machinery is b2-based); the selection-aware bootstrap + `selection_risk_basis="ci_upper"` are skipped loudly (b2-basis diagnostics); bootstrap `risk_ci_*` stays on the b2 basis (HRI has no CI yet — documented limitation); `run_allocation` warns per hri-optimized segment (b2-only tool). Reproducibility `Headline` gains additive `risk_indicator`+`hri_pct` fields (`risk_pct` meaning frozen as b2 forever; legacy references compare unchanged); backtest rows gain `in_sample_hri_pct`/`oot_realized_hri_pct` (drift verdict stays b2). Flipping `risk_indicator` changes cutoffs — M1/M5-style validation required.

**Rejection metrics.** Two €-weighted (on `oa_amt_demand`, through-the-door) rates sit side by side everywhere rejection rate appears (Exec-Summary KPI cards + Scenario Overview + Top-Movers tables, per-segment RP sheets, both HTML reports; Main + MR; Actual + simulated): **Rejection Rate** (final booked-status basis) and **System Rejection Rate** (upstream `se_decision_id == ko`, `SystemDecision` enum). The simulated system rate mirrors the optimum-rejection logic on `se_decision_id`: KO = fails-cutoff **OR** (`ko` AND reason ≠ `09-score`) — below-cutoff `ok`/`rv` → KO, above-cutoff score-`ko` → OK, above-cutoff non-score `ko` stays KO. Computed in `src/audit.py` (`_system_rejection_amounts_from_audit`, set onto the summary table by `reconcile_risk_production_summary_with_audit`); the loan-level audit must carry `se_decision_id` (added to `generate_audit_table`'s columns). Purely additive to the existing rejection rate; degrades to no value when `se_decision_id` is absent. `scripts/preview_exec_summary_kpis.py` renders the Exec-Summary KPI strip to a PNG for quick visual QA of the layout.
