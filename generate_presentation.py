...
def _draw_header(slide, title, subtitle=""):
    ...
    # Blue header bg
    shape = slide.shapes.add_shape(1, Inches(0), Inches(0), SLIDE_WIDTH, Inches(1.7))
    shape.fill.solid()
    shape.fill.fore_color.rgb = DARK_BLUE
    shape.line.fill.background()

    # Title
    _add_textbox(slide, Inches(0.8), Inches(0.35), Inches(11), Inches(0.7), title, font_size=36, color=WHITE, bold=True)
    ...
    # Green accent bar
    bar = slide.shapes.add_shape(1, Inches(0), Inches(1.7), SLIDE_WIDTH, Inches(0.08))
    bar.fill.solid()
...
def _add_table(slide, left, top, width, rows_data, col_widths=None, font_size=11, header_font_size=12):
...
def _arrow(slide, left, top):
    """Small right arrow."""
    _add_textbox(
        slide, left, top, Inches(0.3), Inches(0.35), "\u2192", font_size=18, color=GREY, alignment=PP_ALIGN.CENTER
    )
...
def slide_agenda(prs):
    ...
    tf = _add_textbox(slide, Inches(1.5), Inches(2.1), Inches(10), Inches(5), "", font_size=15)
    for i, item in enumerate(items):
        _add_para(tf, f"  {i + 1}.   {item}", font_size=15, color=BLACK, space_before=Pt(6))
...
def slide_populations_risk(prs):
    ...
    _add_table(
        ...
        [
            ...
            [
                ...
                "Rejected by credit score cutoff (09-score)",
                "Candidates for cutoff changes; outcomes inferred",
            ],
            [
                "Policy-rejected",
                "Rejected for non-score reasons (08-other)",
                "Excluded -- rejection unrelated to creditworthiness",
            ],
        ],
    )
    ...
    _add_para(
        tf3,
        "Normalizes to 12-month equivalent across different observation periods.",
        font_size=11,
        color=GREY,
        space_before=Pt(4),
    )
...
def slide_preprocessing(prs):
    ...
    _add_para(
        tf2, "Weighted Spearman correlation on booked data (weighted by production).", font_size=12, space_before=Pt(6)
    )
    ...
    _add_para(
        tf2, "Can be overridden via [preprocessing.directions] config.", font_size=11, color=GREY, space_before=Pt(6)
    )
    ...
    _code_box(
        ...
        "bin_edges | max_bins # Fixed edges or supervised learning",
        font_size=11,
    )
    ...
    _add_para(tf5, "Compares AUC-based Gini for raw score vs. binned score.", font_size=12, space_before=Pt(6))
    _add_para(tf5, "Reports gini_raw, gini_binned, and retention %.", font_size=12, space_before=Pt(3))
...
def slide_risk_adjustment(prs):
    ...
    _bullet(
        tf, "Separate stress per grid cell (fallback to global for bins < 20 obs)", bold_prefix="Per-bin:", font_size=12
    )
    ...
    _bullet(
        tf,
        "Per-bin mode: computes tasa_fin per grid cell (bins < 10 obs fall back to global)",
        bold_prefix="per_bin_tasa_fin:",
        font_size=12,
    )
    ...
    _add_para(
        tf2,
        "Booked data: no adjustments needed -- observed values used directly.",
        font_size=11,
        color=GREY,
        space_before=Pt(6),
    )
    _add_para(
        tf2,
        "Double-counting warning: using global stress + parceling may overcorrect. "
...
def slide_inference(prs):
    ...
    _add_para(
        tf4,
        "Among models within 1 SE of the best CV RMSE, select the simplest. Applied twice:",
        font_size=12,
        space_before=Pt(6),
    )
    ...
    _add_para(
        tf4, "Prevents overfitting from unnecessarily complex models.", font_size=11, color=GREY, space_before=Pt(6)
    )
...
def slide_optimization(prs):
    ...
    _add_para(
        tf,
        "If a riskier cell is rejected, all cells riskier along that dimension must also be rejected.",
        font_size=12,
        space_before=Pt(6),
    )
    _add_para(
        tf,
        'Creates a "staircase" acceptance pattern. Enforced marginally (per dimension independently).',
        font_size=11,
        color=GREY,
        space_before=Pt(6),
    )
    ...
    _add_para(
        tf3,
        "Built as sparse CSC matrix for ~100x faster constraint setup.",
        font_size=11,
        color=GREY,
        space_before=Pt(8),
    )
...
def slide_pareto(prs):
    ...
    _bullet(
        tf, "Full dominance verification: remove any remaining dominated points", bold_prefix="Step 5:", font_size=12
    )
...
def slide_reject_inference(prs):
    ...
    _add_para(
        tf,
        "Risk models trained on booked data systematically underestimate "
...
def slide_scenarios_bootstrap(prs):
    ...
    _add_para(
        tf,
        "Record-level resampling of the booked portfolio (default: 1,000 iterations). "
    ...
    _add_para(
        tf3,
        "Automated search over (uplift_factor, max_risk_multiplier) to minimize "
    ...
    _bullet(
        tf3,
        "Selection: min calibration error among feasible solutions; ties within 5% broken by max production",
        font_size=12,
    )
    _bullet(
        tf3,
        "MR validation: reports degradation_ratio = mr_error / main_error (near 1.0 = temporally stable)",
        font_size=12,
    )
...
def slide_stability_mr(prs):
    ...
    _add_para(
        tf3,
        "H3\u2192H6 extrapolation methods: linear, power, logistic, or auto "
...
def slide_sensitivity(prs):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    _draw_header(
        slide, "Sensitivity, Marginal Impact & Fixed Cells", "Robustness analysis and interactive re-optimization"
    )
    ...
    _add_para(
        tf,
        "Perturbs risk indicator (todu_30ever_h6_rep) by configurable "
    ...
    _add_para(
        tf2,
        "For each cell, computes the exact production and risk change from flipping its accept/reject status.",
        font_size=12,
        space_before=Pt(6),
    )
    ...
    _add_para(
        tf2,
        "Computed analytically from baseline sums -- no re-solving needed.",
        font_size=11,
        color=GREY,
        space_before=Pt(6),
    )
    ...
    _add_para(
        tf3,
        "Individual cells can be pinned as forced-accept or forced-reject. "
    ...
    _bullet(
        tf3,
        "Interactive: Pin Mode in dashboard Cutoff Explorer (click cells to cycle: unpinned \u2192 accept \u2192 reject \u2192 unpinned)",
        font_size=12,
    )
    _add_para(
        tf3,
        "Returns None if pins are contradictory or make risk target infeasible.",
        font_size=11,
        color=GREY,
        space_before=Pt(6),
    )
...
def slide_batch_allocation(prs):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    _draw_header(
        slide, "Batch Processing & Global Allocation", "Multi-segment execution and portfolio-level optimization"
    )
    ...
    _add_para(
        tf2,
        "Given N segment Pareto frontiers, select one operating point per "
...
def slide_trends(prs):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    _draw_header(slide, "Trend Analysis & SPC Anomaly Detection", "Monthly monitoring with statistical process control")
    ...
    _add_para(
        tf,
        "Output: monthly_metrics_{segment}.csv + interactive Plotly charts.",
        font_size=11,
        color=GREY,
        space_before=Pt(8),
    )
    ...
    _add_para(
        tf2, "Uses a one-period-lagged rolling estimator to avoid lookahead bias:", font_size=12, space_before=Pt(6)
    )
    ...
    _add_para(
        tf2,
        "Robust to outliers (median + MAD instead of mean + std). "
...
def slide_reporting(prs):
    ...
    _add_para(tf, "Self-contained Jinja2 template with embedded Plotly charts:", font_size=12, space_before=Pt(6))
    ...
    _add_table(
        ...
        [
            ...
            [
                "Results Explorer",
                "dashboard.py (Dash, port 8050)",
                "Scenario comparison, cutoff explorer, pin mode, uncertainty overlays",
            ],
            [
                "Global Allocator",
                "interactive_allocator.py (Dash, port 8051)",
                "Real-time portfolio allocation, per-segment risk sliders",
            ],
...
def slide_config_summary(prs):
    ...
    _add_para(
        tf2,
        "Any config.toml field can be overridden per segment via recursive merge.",
        font_size=12,
        space_before=Pt(6),
    )
...
def slide_summary(prs):
    ...
    _add_textbox(
        slide, Inches(1), Inches(0.8), Inches(11), Inches(0.8), "Summary", font_size=40, color=WHITE, bold=True
    )
    ...
    items = [
        ...
        "799 tests, Pydantic config, 50+ configurable parameters, Docker-ready deployment",
    ]
